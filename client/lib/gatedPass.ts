...
export interface GatedPassOptions {
  /** PNG data-url of the QR code image */
  qrImage: string;
  eventTitle: string;
  participantName: string;
  registrationId: string;
  /** External visitors get an extra "show this at the campus gate" line */
  isOutsider?: boolean;
}

/**
 * Generates and downloads the official "GATED" entry-pass PDF.
 *
 * This is the single source of truth for the gate-pass layout, used by both the
 * Profile page (QRCodeDisplay) and the post-registration ticket page so both
 * download flows produce an identical document.
 */
export async function downloadGatedPass({
  qrImage,
  eventTitle,
  participantName,
  registrationId,
  isOutsider = false,
}: GatedPassOptions): Promise<void> {
