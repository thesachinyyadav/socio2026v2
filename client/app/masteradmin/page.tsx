  ...
  const fetchClubs = async (options?: { unpaged?: boolean }) => {
    try {
      ...
      if (clubStatusFilter !== "all") {
        ...
        filtered = filtered.filter(c => c.club_registrations === neededRegStatus);
      }

      filtered.sort((a, b) => {
        let valA: any = a.club_name;
        let valB: any = b.club_name;
      ...
      const paginated = options?.unpaged ? filtered : filtered.slice(startIndex, startIndex + ITEMS_PER_PAGE);
    ...
    <div className="flex items-center justify-between px-6 py-4 bg-gray-50 border-t border-gray-200">
      ...
      <div className="flex gap-2">
        <button
          onClick={onPrev}
          disabled={!hasPrev}
          className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${hasPrev
        ...
        <button
          onClick={onNext}
          disabled={!hasNext}
          className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${hasNext
          ...
          {sidebarNav.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium transition-all relative group ${isActive
              ...
              >
                ...
                {"count" in item && item.count !== undefined && item.count > 0 && (
                  <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${isActive ? "bg-[#154cb3]/10 text-[#154cb3]" : "bg-slate-100 text-slate-500"
                    }`}>{item.count}</span>
                )}
              </button>
            );
              ...
              <div className="flex flex-wrap gap-3 mb-5">
                ...
                {(["all", "UNDER_REVIEW", "APPROVED", "REJECTED"] as const).map(f => (
                  <button
                    ...
                    className={`px-4 py-2 text-xs font-semibold rounded-lg border transition-colors ${pipelineFilter === f
                          ...
                          return (
                            <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                              ...
                              <td className="px-4 py-3">
                                <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide ${isFest ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"
                                  }`}>
                                  {item.entity_type || "—"}
                                </span>
                              </td>
                              ...
                              <td className="px-4 py-3">
                                <span className={`inline-flex px-2.5 py-1 rounded-full text-[11px] font-bold ${statusColors[statusToken] || "bg-slate-100 text-slate-600"
                                  }`}>
                              ...
                              <tr key={club.club_id} className="hover:bg-gray-50 transition-all duration-200">
                                ...
                                <td className="px-6 py-5 text-right align-top">
                                  <div className="flex flex-wrap items-center justify-end gap-2">
                                    <a
                                      href="#"
                                      onClick={(e) => { e.preventDefault(); toast.success("Edit organization module coming soon!"); }}
