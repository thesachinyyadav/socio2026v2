...
async function proxyRequest(
  ...
  const backendUrl = `${getBackendBaseUrl()}/api/notifications/${backendPath}${request.nextUrl.search}`;
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  try {
    ...
    return NextResponse.json({ error: error?.message || "Failed to reach notifications service" }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  try {
    ...
    console.error("Notifications proxy DELETE error:", error);
    return NextResponse.json({ error: error?.message || "Failed to reach notifications service" }, { status: 500 });
  }
}